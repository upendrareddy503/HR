...
namespace BLL
{
    public class Shift:Group
    {
        ...
        {            
            SqlParameter[] parm = new SqlParameter[3];
            parm[0] = da.AddSPParameter("Tai_Uaseid", UserId, ParameterDirection.Input, DbType.Int32, 10);
            parm[1] = da.AddSPParameter("Tai_Date", DateTime.Today, ParameterDirection.Input, DbType.DateTime, 10);
            parm[2] = da.AddSPParameter("Flag", 0, ParameterDirection.Input, DbType.Int32, 10);
            DataTable dt = da.Sp_Datatable("Usp_Attendance_In_Out", parm);
            List<Shift> obj_Shift_lst = new List<Shift>();
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                Shift obj_Shift = new Shift();
                
                obj_Shift.ShiftInTime = dt.Rows[i]["Tai_Date"].ToString();               
                obj_Shift_lst.Add(obj_Shift);
            }
            return obj_Shift_lst;
        }
